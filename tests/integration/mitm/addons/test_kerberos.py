...
if not os.path.exists((os.getenv("KRB5CCNAME") or "").split("DIR:")[1]):
    pytest.skip(
        "no credentials cache in $KRB5CCNAME, can't run integration test",
...
def mitmproxy_command(port, env=None):
    ...
    process = subprocess.Popen(
        [
            ...
            "--no-web-open-browser",
            "--proxyauth",
            "@" + env["PROXY_HTPASSWD_PATH"],
            "-s",
            kerberos.__file__,
            "--set",
            "ssl_verify_upstream_trusted_ca=" + env["SSL_CERT_FILE"],
            "--listen-port",
            str(port),
            "--web-port",
            str(get_free_port_number()),
            "--set",
            "kerberos_realm=" + env["KERBEROS_REALM"],
            "--set",
            "kerberos_spnego_codes=401",
            "--set",
            "kerberos_knox_urls=",
            "--set",
            "kerberos_keytabs_path=" + env["KEYTABS_PATH"],
            "--set",
            "hosts_mappings=" + (env.get("HOSTS_MAPPINGS") or ""),
        ],
        env=env,
    )
    try:
